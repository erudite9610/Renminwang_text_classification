...
class Simple_model(torch.nn.Module):
    def __init__(self):
        ...
        self.lstm_1 = torch.nn.LSTM(input_size=lib.len_word_vector, hidden_size=lib.lstm_unit_num_1,
                                    ...
                                    dropout=lib.dropout)
        # self.full_connection_layer_1 = torch.nn.Linear(lib.lstm_unit_num_1 * 2, 2)
        self.full_connection_layer_1 = torch.nn.Linear(lib.lstm_unit_num_1 * 2, lib.full_connection_layer_1_output)
        # self.lstm_2 = torch.nn.LSTM(input_size=lib.full_connection_layer_1_output, hidden_size=lib.lstm_unit_num_2,
        #                             num_layers=lib.lstm_num_layer_2, bidirectional=False, batch_first=True, dropout=lib.dropout)
    ...
    def forward(self, input):
        ...
        x, (hidden_layer, cell) = self.lstm_1(x)
        # 获取两个方向最后一次的output
...
x = Simple_model()
x = torch.zeros((lib.batch_size, lib.max_len), dtype=torch.long)
MyConvNetVis = make_dot(simple_model (x),show_attrs=False, params=dict(simple_model .named_parameters()))
MyConvNetVis.format = "png"
MyConvNetVis.directory = "models"
MyConvNetVis.view()
...
def train(epoch):
    for idx, (input, label) in enumerate(get_dataloader(train=True)):
        ...
        loss = torch.nn.functional.nll_loss(output, label.to(device))
        loss.backward()
        optimizer.step()
...
def eval():
    ...
    for index, (input, label) in enumerate(get_dataloader(train=False)):
        ...
        with torch.no_grad():
            ...
            current_loss = torch.nn.functional.nll_loss(output, label)
